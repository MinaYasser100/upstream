...
class ThinkWithUsView extends StatelessWidget {
  const ThinkWithUsView({super.key});

  @override
  Widget build(BuildContext context) {
    final thinking = ConferenceData.thinking.trim().split('\n\n');
    return Scaffold(
      body: CustomBackground(
        child: CustomScrollView(
          physics: const AlwaysScrollableScrollPhysics(),
          slivers: [
            const CustomTitleView(title: 'فكر معانا'),
            ContentListBodyView(stringList: thinking),
            const CustomDivider(),
          ],
        ),
      ),
